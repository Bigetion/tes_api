...
class Crud2_module extends Main {
    ...
    function index() {
		$this->load->data('header_text','Build CRUD2 Module');
		...
		if(segment(4)=="") $this->load->app_view('app/crud2_module/view_build');
		else{
			$a = load_file('project');
			if(in_array(segment(4),$a) && count($a)>2){
				$this->load->app_view('app/crud2_module/view_build2');
			}
			else{
				$this->load->app_view('app/crud2_module/view_build');	
			}
		}
    ...
    function form_valid() {
        ...
        $this->load->app_view('app/crud2_module/view_form_valid');
    }
    ...
    function create() {
        ...
        if (isset($this->set->data['error'])) {
            ...
            $this->set->show_page('#form_valid', base_url . 'crud2_module/form_valid/' . $project . '/' . $modul . '/' . $tabel . '/' . $title);
        }
    }
    ...
    function build() {
        ...
        $this->module->build_crud2_controller($project, $config);
