...
class Static_page_module extends Main {
    ...
    function index() {
		...
		$this->load->data('header_text','Build Static Page Module ');
		
        $this->load->app_view('header-footer/app_header');
		if(segment(4)=="") $this->load->app_view('app/static_page_module/view_build');
		else{
			$a = load_file('project');
			if(in_array(segment(4),$a) && count($a)>2){
				$this->load->app_view('app/static_page_module/view_build2');
			}
			else{
				$this->load->app_view('app/static_page_module/view_build');	
			}
		}
    ...
    function build() {
        ...
        if (isset($this->set->data['error'])) {
            ...
            $this->module->build_page_static_controller($project, $config);
